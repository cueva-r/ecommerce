                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Lista de código de descuento</h3>
                            </div>

                            <div class="card-body">
                                <table class="table table-striped" id="codigoDescuento">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>Nombre</th>
                                            <th>Tipo</th>
                                            <th>Porcentaje / Cantidad</th>
                                    ...
                                    <tbody>
                                        @foreach ($getRecord as $valor)
                                            <tr>
                                                ...
                                                <td>{{ $valor->nombre }}</td>
                                                <td>{{ $valor->tipo }}</td>
                                                <td>{{ $valor->porcentaje_cantidad }}</td>
                                                <td>{{ date('d-m-Y', strtotime($valor->fecha_expiracion)) }}</td>
                                                <td>{{ $valor->estado == 0 ? 'Activo' : 'Inactivo' }}</td>
                                                {{-- <td>{{ date('d-m-Y', strtotime($valor->created_at)) }}</td> --}}
                                                <td>{{ $valor->created_at->diffForHumans() }}</td>
                                                <td>
                                                    <a href="{{ url('admin/codigo_descuento/editar/' . $valor->id) }}"
                                                        class="btn btn-outline-primary btn-sm">
                                                        <i class="fa-solid fa-pen-to-square"></i>
                                                    </a>
                                                    <a href="{{ url('admin/codigo_descuento/eliminar/' . $valor->id) }}"
