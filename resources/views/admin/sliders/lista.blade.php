            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6" style="text-align: right">
                        <a href="{{ url('admin/sliders/agregar') }}" class="btn btn-primary">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">Lista sliders</h3>
                            </div>

                            <div class="card-body">
                                <table class="table table-striped" id="sliders">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>Imágen</th>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if (!empty($valor->getImagen()))
                                                        <img src="{{ $valor->getImagen() }}" style="width: 200px">
                                                    @endif
                                                </td>
                                                <td>{{ $valor->titulo }}</td>
                                                <td>{{ $valor->nombre_button }}</td>
                                                <td>{{ $valor->link_button }}</td>
                                                <td>{{ $valor->estado == 0 ? 'Activo' : 'Inactivo' }}</td>
                                                {{-- <td>{{ date('d-m-Y', strtotime($valor->created_at)) }}</td> --}}
                                                <td>{{ $valor->created_at->diffForHumans() }}</td>
                                                <td>
                                                    <a href="{{ url('admin/sliders/editar/' . $valor->id) }}"
                                                        class="btn btn-outline-primary btn-sm">
                                                        <i class="fa-solid fa-pen-to-square"></i>
                                                    </a>
                                                    <a href="{{ url('admin/sliders/eliminar/' . $valor->id) }}"
